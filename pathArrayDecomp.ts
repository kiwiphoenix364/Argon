function decompString(data: string) {
    let charCounterForVersion = 0
    while (data.charAt(charCounterForVersion) != ")") {
        charCounterForVersion++
    }
    charCounterForVersion++
    //Decompiling code for version 1.0
    if(data.substr(1, charCounterForVersion - 2) === "1.0") {
        ...
        let dValue: number
        let oValue: number
        for (let i = charCounterForVersion; i < data.length; i++) {
            //Handle skips
